...
| Desktop layout            | Ensure the site layout renders correctly on standard desktop resolutions.                             |     Pass     |                          |                      |
| Mobile responsiveness     | Check layout on smaller screens (mobile devices) for proper scaling, spacing, and alignment.          |     Pass     |                          |                      |
| Tablet responsiveness     | Verify layout adjusts properly on tablets (e.g., iPads).                                              |     Pass     |                          |                      |
...
| Visual consistency        | Ensure the site's Norse mythology theme is maintained throughout fonts, colours, and imagery.         |     Pass      |                          |                      |
| Logo visibility           | Confirm the logo displays clearly and is properly aligned across all pages.                          |      Pass      |                          |                      |
| Background elements       | Verify that background images, gradients, or patterns display without distortion or tiling issues.    |     Pass      |                          |                      |
| Colour contrast           | Check that text is easily readable against the background (meet WCAG accessibility guidelines).       |     Fail      |  Text and background not contrasting well in certain areas    |                      |
| Image quality             | Verify images and illustrations (e.g., gods, mythology elements) are clear and optimised for web.     |     Fail      |  Background images blurry in parts       |                      |
| **3. Navigation Menu** |
| Navigation visibility     | Ensure the navigation menu is present and visible on all pages.                                       |     Pass      |                          |                      |
| Menu links                | Confirm all menu links navigate to the correct sections/pages (e.g., homepage, characters, history).  |     Pass      |                          |                      |
| Active menu styling       | Check if the active section/page is visually highlighted in the menu.                                 |               |                          |                      |
| Mobile menu functionality | Ensure the menu works correctly on mobile devices (e.g., hamburger menu expands/collapses).           |     Pass      |                          |                      |
| Smooth scrolling          | Confirm that menu links for internal sections (e.g., "About", "Characters") enable smooth scrolling.  |     Pass      |                          |                      |
...
| Hover effects             | Verify hover effects (e.g., on buttons, links, or images) work correctly and align with the theme.    |     Pass      |                          |                      |
| Call-to-action buttons    | Check that all buttons (e.g., "Learn More", "Explore") navigate or perform actions as expected.       |     Pass      |                          |                      |
| Animation/scroll effects  | Ensure animations and scroll effects (if any) function correctly and enhance the user experience.     |     Pass      |                          |                      |
| **7. Accessibility** |
